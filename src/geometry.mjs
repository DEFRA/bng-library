...
// in the `gpkg-io` subpath; the index constants stay here because they're
...
export function polygonCentroid(ring) {
  ...
  return [
    cx / (CENTROID_DIVISOR * twiceArea),
    cy / (CENTROID_DIVISOR * twiceArea)
  ]
}
...
export function convexHull(points) {
  ...
  for (const p of pts) {
    while (
      lower.length >= 2 &&
      cross(lower.at(HULL_SECOND_LAST), lower.at(HULL_LAST), p) <= 0
    ) {
      lower.pop()
    }
    lower.push(p)
  ...
  for (let i = pts.length - 1; i >= 0; i--) {
    ...
    while (
      upper.length >= 2 &&
      cross(upper.at(HULL_SECOND_LAST), upper.at(HULL_LAST), p) <= 0
    ) {
      upper.pop()
    }
    upper.push(p)
...
export function generateIrregularPolygon(
  ...
  cy,
  radius,
  numPoints = HULL_POINT_COUNT_DEFAULT
) {
  const pts = []
  for (let i = 0; i < numPoints; i++) {
    const angle = randomAngle()
    const r =
      radius * (ANNULUS_INNER_FRACTION + randomFraction() * ANNULUS_OUTER_SPAN)
...
function splitPolygonRandom(ring) {
  ...
  const offset = randBetween(
    minSd * CHORD_OFFSET_RANGE_FRACTION,
    maxSd * CHORD_OFFSET_RANGE_FRACTION
  )
 ...
 */
function searchChordOffsetForArea(
  ring,
  normal,
  lo,
  hi,
...
function tryCarveTargetArea(ring, targetArea) {
  ...
  const offset = searchChordOffsetForArea(
    ring,
    normal,
    minSd,
    maxSd,
  ...
  if (
    piece &&
    rest &&
    polygonArea(piece) > MIN_VALID_SUB_POLYGON_AREA &&
    polygonArea(rest) > MIN_VALID_SUB_POLYGON_AREA
  ) {
...
export function partitionPolygon(
  ring,
  n,
  maxRetriesPerSplit = DEFAULT_PARTITION_RETRIES_PER_SPLIT
) {
  const parcels = [ring]
  while (parcels.length < n) {
    ...
    for (let r = 0; r < maxRetriesPerSplit; r++) {
      ...
      if (
        a &&
        b &&
        polygonArea(a) > MIN_VALID_SUB_POLYGON_AREA &&
        polygonArea(b) > MIN_VALID_SUB_POLYGON_AREA
      ) {
        split = [a, b]
        break
      }
...
export function pickInteriorPoint(
  boundaryRing,
  maxAttempts = DEFAULT_INTERIOR_POINT_ATTEMPTS
) {
