 ...
 */
export function generateBaselineFile(
  outPath,
  _workbook,
  baselineRows,
  plan,
 ...
 */
export function generatePostInterventionFile(
  outPath,
  workbook,
  postRows,
  baselineGeom,
  ...
  const hedgeCoords = derivePostInterventionLinearCoords(
    ring,
    baselineGeom.hedgeCoordsByRef,
    postRows.hedgerows
  )
  const hedgesWritten = writeHedgerowsPostIntervention(
    db,
    hedgeCoords,
    postRows.hedgerows
  )

  const riverCoords = derivePostInterventionLinearCoords(
    ring,
    baselineGeom.riverCoordsByRef,
    postRows.rivers
  )
  const riversWritten = writeRiversPostIntervention(
    db,
    riverCoords,
    postRows.rivers
  )
...
export function generateFromWorkbook(
  ...
  const postRows =
    mode === MODE_BASELINE
      ? null
      : buildPostInterventionRows(workbook, { strict })

  const habitatRowsForSizing = baselineRows.habitats.map((r) => ({
    area: r.area
  }))
  ...
  const baselineDest =
    mode === MODE_POST_INTERVENTION ? IN_MEMORY_DB : outPaths.baseline
  const baselineGeom = generateBaselineFile(
    baselineDest,
    workbook,
    baselineRows,
    plan,
