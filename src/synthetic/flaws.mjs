...
export const FLAWS = {
  ...
  'bowtie-parcel': {
    ...
    apply(s) {
      s.parcels.push(
        bowtieRing(s.cx, s.cy + BOWTIE_PARCEL_DY, BOWTIE_PARCEL_HALF)
      )
    }
  },
  'overlapping-parcels': {
  ...
  'parcel-outside-redline': {
    ...
    apply(s) {
      s.parcels.push(
        badSquareRing(s.cx + OUTSIDE_PARCEL_DXY, s.cy + OUTSIDE_PARCEL_DXY)
      )
    }
  },
  sliver: {
    description: 'a hairline habitat parcel too small to be a real habitat',
    errorCode: 'AREA_PARCELS_TOO_SMALL',
    apply(s) {
      const x = s.cx + SLIVER_PARCEL_DX
      const y = s.cy + SLIVER_PARCEL_DY
      s.parcels.push(
        rectRing(x, y, x + SLIVER_PARCEL_LENGTH, y + SLIVER_PARCEL_WIDTH)
      )
    }
  },
  'hedgerow-outside': {
  ...
  'iggi-outside': {
    ...
    apply(s) {
      s.iggis.push(
        badSquareRing(
          s.cx - IGGI_OUTSIDE_OFFSET,
          s.cy - IGGI_OUTSIDE_OFFSET,
          IGGI_HALF
        )
      )
    }
  ...
  'distinctiveness-out-of-scope': {
    description:
      'habitat types whose reference-data distinctiveness is rejected by the BNG Beta service',
    ...
    attributeOverride: {
      ...
      perRow: [
        ...
        {
          habitatFullName: 'Grassland - Traditional orchards',
          retention: 'Retained'
        } // High
      ]
    }
...
function assertCategoryConflicts(buckets, bad) {
  for (const [a, b] of EXCLUSIVE_PAIRS) {
    ...
    if (bad && (a === CATEGORY_GEOMETRIC || b === CATEGORY_GEOMETRIC)) {
      error(
        `--bad cannot be combined with ${CATEGORY_LABEL[nonGeometric]} flaws.`
      )
    } else {
      error(
...
function assertAttributeLayerNotEmptied(attributeNames, emptyNames) {
  for (const name of attributeNames) {
    ...
    const conflictingEmpty = emptyNames.find(
      (n) => FLAWS[n].emptyLayer === targetLayer
    )
    if (conflictingEmpty) {
      error(
