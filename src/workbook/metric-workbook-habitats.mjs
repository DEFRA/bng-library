...
export function readBaselineHabitats(workbook, summary) {
  ...
  const { dataStart, header } = findHeader(aoa, [
    HDR_REF,
    HDR_BROAD_HABITAT,
    HDR_AREA_HECTARES
  ])
  if (dataStart < 0) {
    summary.warnings.push(
      `Could not locate header row in ${SHEETS.habitatsBaseline}`
    )
    return []
  }
  ...
  for (let r = dataStart; r < aoa.length; r++) {
    ...
    if (action.skipBlankArea) {
      summary.skipped.push({
        sheet: SHEETS.habitatsBaseline,
        row: r + 1,
        reason: 'blank area'
      })
    } else if (action.decision) {
      out.push(
        buildBaselineHabitatEntry(aoa[r], action.decision, out.length, cols)
      )
    } else {
...
export function readCreatedHabitats(workbook, summary) {
  ...
  if (dataStart < 0) {
    summary.warnings.push(
      `Could not locate header row in ${SHEETS.habitatsCreation}`
    )
    return []
  }
...
function resolveEnhancementCols(header) {
  ...
  return {
    ...
    cAdvance: col(
      ...
      'Habitat enhanced in advance'
    ),
    cDelay: col(
      idx,
      'Delay in starting enhancement (years)',
      'Delay in starting habitat enhancement',
...
function buildEnhancementEntry(
  row,
  { baselineRef, propBroad, propType },
  cols
) {
  return {
    baselineRef,
