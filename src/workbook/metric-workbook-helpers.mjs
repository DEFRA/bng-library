...
// Watercourse-only encroachment columns on C-1 / C-2 / C-3. The
// "Watercourse encroachment" / "Riparian encroachment" group labels sit on
// the top header row, but findHeader merges the two-row header preferring
// the bottom row, so the resolved header carries the bottom-row labels
// below — these are what col() matches. Absent on hedge sheets (col()
// returns -1 there).
