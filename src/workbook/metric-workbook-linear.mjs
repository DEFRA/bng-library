...
function resolveLinearCols(header, typeHeader, withFate) {
  ...
  return {
    ...
    fate: {
      cLenRetained: withFate
        ? col(idx, 'Length retained', 'Length Retained')
        : -1,
      cLenEnhanced: withFate
        ? col(idx, 'Length enhanced', 'Length Enhanced')
        : -1,
...
export function readLinearFeatures(
  workbook,
  sheetName,
  kind,
  summary,
  ...
  const { base: baseCols, fate: fateCols } = resolveLinearCols(
    header,
    typeHeader,
    withFate
  )
