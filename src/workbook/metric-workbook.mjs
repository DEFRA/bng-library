...
export function detectMetricVersion(workbook) {
  ...
  if (
    names.has(SHEETS.habitatsBaseline) &&
    names.has(SHEETS.habitatsCreation)
  ) {
    return METRIC_V4
  }
...
function normaliseWorkbook(workbook, sourceLabel) {
  ...
  const hedgerows = {
    baseline: readLinearFeatures(
      workbook,
      SHEETS.hedgesBaseline,
      'hedge',
      summary,
      { withFate: true }
    ),
    created: readLinearFeatures(
      workbook,
      SHEETS.hedgesCreation,
      'hedge',
      summary
    ),
    enhancements: readLinearEnhancements(
      workbook,
      SHEETS.hedgesEnhancement,
      summary
    )
  }
  const watercourses = {
    ...
    created: readLinearFeatures(
      workbook,
      SHEETS.watercoursesCreation,
      'river',
      summary
    ),
    enhancements: readLinearEnhancements(
      workbook,
      SHEETS.watercoursesEnhancement,
      summary
    )
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!file) {
    console.error(
      'Usage: node packages/bng-lib/src/workbook/metric-workbook.mjs <workbook.xlsx>'
    )
    process.exit(1)
  }
