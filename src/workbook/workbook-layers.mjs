...
import { gpkgAreaRetention } from '../retention.mjs'
...
function planBaselineCellPartition(baseline, cellArea, group) {
  ...
  const createdEntries = group.filter(
    (entry) => entry.row.retention === 'Created'
  )
 ...
 */
function distributeBaselineCell(
  cell,
  subTargets,
  subAssignments,
  cellsForPost,
  ...
  const subCells =
    subTargets.length === 1 ? [cell] : partitionPolygonByAreas(cell, subTargets)
...
function warnOrAssignOrphanCell(
  ...
  if (!cell) {
    warnings.push(
      `created habitat ${unassigned.row.ref}: orphan lost-area pool exhausted — geometry omitted`
    )
    return
  }
...
export function derivePostInterventionHabitatCells(
  ...
  const { groupedByBaseline, unassignedCreated } =
    groupPostRowsForGeometry(postRows)
  ...
  for (const b of baselineRowsRaw) {
    ...
    const { subTargets, subAssignments } = planBaselineCellPartition(
      b,
      polygonArea(cell),
      group
    )
    distributeBaselineCell(
      cell,
      subTargets,
      subAssignments,
      cellsForPost,
