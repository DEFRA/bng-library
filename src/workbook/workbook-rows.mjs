...
export function makeRef(prefix, baselineRef, suffix) {
  const num = String(baselineRef).padStart(
    FEATURE_REF_PAD,
...
export function fateReconciles(total, retained, enhanced, lost) {
  ...
  const tol = Math.max(
    total * FATE_AREA_RELATIVE_TOLERANCE,
    FATE_AREA_ABSOLUTE_FLOOR_HA
  )
...
export function buildBaselineRows(wb, { strict = false } = {}) {
  ...
  for (const b of wb.habitats.baseline) {
    if (strict && !isHabitatConditionValid(b.broad, b.type, b.condition)) {
      skipReasons.push(
        `baseline habitat ref ${b.ref}: invalid (habitat, condition) under --strict-habitats`
      )
      continue
    }
...
function filterEligibleCreated(created, strict, skipReasons) {
  ...
  for (let i = 0; i < created.length; i++) {
    ...
    if (strict && !isHabitatConditionValid(c.broad, c.type, c.condition)) {
      skipReasons.push(
        `created habitat ref ${c.ref}: invalid (habitat, condition) under --strict-habitats`
      )
      continue
    }
    idxs.push(i)
 ...
 */
function assignCreatedToBaselines(
  eligibleBaselines,
  created,
  eligibleCreatedIdxs
) {
...
function findBestBaselineForCreated(lostRemaining, needed) {
  ...
  for (const [ref, remaining] of lostRemaining) {
    if (
      remaining + FATE_AREA_ABSOLUTE_FLOOR_HA >= needed &&
...
function planEnhancementProposed(b, baselineRef, habEnh, strict, skipReasons) {
  ...
  if (
    ...
    skipReasons.push(
      `enhanced habitat ref ${baselineRef}: invalid proposed (habitat, condition) under --strict-habitats`
    )
    return null
  }
  return proposed
...
function emitRowsForBaseline(
  ...
  const proposed = planEnhancementProposed(
    b,
    baselineRef,
    habEnh,
    strict,
...
function buildHabitatPostRows(wb, strict, skipReasons, warnings) {
  ...
  const eligibleBaselines = filterEligibleBaselines(
    wb.habitats.baseline,
    strict,
    skipReasons,
    warnings
  )
  const eligibleCreatedIdxs = filterEligibleCreated(
    wb.habitats.created,
    strict,
    skipReasons
  )
  const createdAssignment = assignCreatedToBaselines(
    eligibleBaselines,
    wb.habitats.created,
    eligibleCreatedIdxs
  )
  ...
  for (const idx of eligibleCreatedIdxs) {
    ...
    habitats.push(
      emitUnassignedCreatedRow(wb.habitats.created[idx], createdSeq)
    )
    createdSeq += 1
  }
  return habitats
...
function warnIfLinearAccountingOff(
  b,
  retainedM,
  enhancedM,
  lostM,
  ...
  if (retainedM > 0) {
    rows.push(
      makeLinearRow(
        ...
        'Retained',
        refPrefix,
        linearSuffix(retainedM, enhancedM, 'Retained')
      )
    )
  }
  if (enhancedM > 0) {
    rows.push(
      makeLinearRow(
        ...
        'Enhanced',
        refPrefix,
        linearSuffix(retainedM, enhancedM, 'Enhanced'),
        enhMap.get(String(b.ref))
      )
    )
  }
  return rows
